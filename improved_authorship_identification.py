...
def predict_author_with_downloaded_model(model_path, author_mappings, device):
    ...
    predicted_author = author_mappings[predicted_class]
...
def make_ml_prediction():
    while True:
        ...
        if option == 'YES':
            ...
            # author_mappings.txt contains lines containg {id, author_name} in each line
            author_mappings = {}
            with open(os.path.join(model_path, "author_mappings.txt"), "r") as file:
                for line in file:
                    key, value = line.strip().split(", ")
                    author_mappings[int(key)] = value
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            predicted_author = predict_author_with_downloaded_model(model_path, author_mappings, device)
            print(f"\nPredicted author: {predicted_author}")
            break
        elif option == 'NO':
...
def main():
    while True:
        ...
        print("Choose an option:")
        print("A: Identify Author using Gemini AI")
        print("B: Identify Author using pre-trained ML model on Famous Authors")
